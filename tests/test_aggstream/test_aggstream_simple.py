...
def test_time_grouper_sum_agg(test_id, row_group_target_size, expected_n_rows, store, seed_path):
    ...
    fp_write(str(seed_path), seed_df, row_group_offsets=row_group_offsets, file_scheme="hive")
    ...
    n_rows_res = store[key].row_group_stats.loc[:, KEY_N_ROWS].to_list()
    ...
    assert store[key].to_pandas().equals(ref_res)
    ...
    # but 'cumsegagg' scope.
    streamagg_md = store[key].key_value_metadata[KEY_AGGSTREAM]
    ...
    fp_write(str(seed_path), seed_df, file_scheme="hive", append=True)
    seed = ParquetFile(str(seed_path)).iter_row_groups()
    ...
    fp_write(str(seed_path), seed_df, file_scheme="hive", append=True)
    seed_pf = ParquetFile(str(seed_path))
...
def test_duration_weighted_mean_from_post(store, seed_path):
    ...
    assert store[key].to_pandas().equals(ref_res_post)
    # Check number of rows of each row groups in aggregated results.
    n_rows_res = store[key].row_group_stats.loc[:, KEY_N_ROWS].to_list()
    n_rows_ref = [3, 2, 2]
...
def test_seed_time_grouper_bin_on_as_tuple(store, seed_path):
    ...
    fp_write(str(seed_path), seed_pdf, row_group_offsets=row_group_offsets, file_scheme="hive")
    seed = ParquetFile(str(seed_path)).iter_row_groups()
    # Streamed aggregation.
    as_.agg(seed=seed, trim_start=True, discard_last=True)
    # Test results
    ...
    fp_write(str(seed_path), seed_pdf2, file_scheme="hive", append=True)
    seed = ParquetFile(str(seed_path)).iter_row_groups()
...
def test_by_callable_wo_bin_on(store, seed_path):
    ...
    fp_write(str(seed_path), seed_pdf, row_group_offsets=13, file_scheme="hive")
    seed = ParquetFile(str(seed_path)).iter_row_groups()
    as_.agg(seed=seed, trim_start=True, discard_last=True)
    # Get reference results, discarding last row, because of 'discard_last'.
    ...
    # Test results
    assert store[key].to_pandas().equals(ref_res_agg)
    # Check metadata.
    streamagg_md = store[key].key_value_metadata[KEY_AGGSTREAM]
    assert streamagg_md[KEY_RESTART_INDEX] == ts[-1]
    assert not streamagg_md[KEY_PRE_BUFFER]
    ...
    fp_write(str(seed_path), seed_pdf2, row_group_offsets=13, file_scheme="hive")
    seed = ParquetFile(str(seed_path)).iter_row_groups()
    # Streamed aggregation.
    as_.agg(seed=seed, trim_start=True, discard_last=True)
    # Get reference results, discarding last row, because of 'discard_last'.
    ...
    # Test results
    assert store[key].to_pandas().equals(ref_res_agg2)
    # Check metadata.
    streamagg_md = store[key].key_value_metadata[KEY_AGGSTREAM]
...
def test_time_grouper_trim_start(store, seed_path):
    ...
    seed_pdf = DataFrame({ordered_on: ts, "val": range(1, len(ts) + 1)})
    fp_write(str(seed_path), seed_pdf, file_scheme="hive")
    seed = ParquetFile(str(seed_path)).iter_row_groups()
    # Streamed aggregation.
    as_.agg(seed=seed, trim_start=True, discard_last=True)
    # Test results.
    ...
    fp_write(str(seed_path), seed_pdf2, file_scheme="hive")
    seed = ParquetFile(str(seed_path)).iter_row_groups()
    # Streamed aggregation.
...
def test_single_row_within_seed(store, seed_path):
    ...
    fp_write(str(seed_path), seed_pdf, row_group_offsets=[0, 5, 6], file_scheme="hive")
    seed = ParquetFile(str(seed_path)).iter_row_groups()
    # Streamed aggregation: no aggregation, but no error message.
    as_.agg(seed=seed, trim_start=True, discard_last=True)
    # Test results.
...
def test_time_grouper_agg_sum_filters_and_no_filter(store):
    ...
    ref_res[SUM] = ref_res[SUM].astype(DTYPE_NULLABLE_INT64)
    assert store[key_a].to_pandas().equals(ref_res)
    # 'key_b'
    ref_res = seed2.loc[seed2["val"] >= 3].groupby(bin_by).agg(**agg).reset_index()
    ref_res[SUM] = ref_res[SUM].astype(DTYPE_NULLABLE_INT64)
    assert store[key_b].to_pandas().equals(ref_res)
...
def test_post_with_warm_up(store):
    ...
    post_buffer = store[key].key_value_metadata[KEY_AGGSTREAM][KEY_POST_BUFFER]
