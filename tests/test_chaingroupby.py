...
@pytest.mark.parametrize(
    "type_, other_type, agg_func1, agg_func2, agg_func3",
    [
        (FLOAT64, INT64, FIRST, MIN, LAST),
        (FLOAT64, INT64, LAST, MAX, SUM),
        (INT64, FLOAT64, FIRST, MIN, LAST),
        (INT64, FLOAT64, LAST, MAX, SUM),
    ],
)
def test_jitted_cgb(type_, other_type, agg_func1, agg_func2, agg_func3):
    # Setup.
    ...
    other_data = zeros(0, dtype=other_type)
    ...
    other_agg_res = zeros(0, dtype=other_type)
    if type_ == FLOAT64:
        config = {
            "data_float": data,
            ...
            "agg_res_int": other_agg_res,
        }
    else:
        config = {
            "data_float": other_data,
    ...
    assert nall(ref_res[(type_, other_type, agg_func1, agg_func2, agg_func3)] == agg_res)
