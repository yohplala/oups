...
@pytest.mark.parametrize(
    "by, end_indices, chunk_labels_refs, next_chunk_starts_refs, n_null_chunks_refs, chunk_ends_refs, buffer_refs",
    [
        (
            ...
            [
                {KEY_LAST_KEY: pTimestamp("2020-01-01 08:10:00")},
                {KEY_LAST_KEY: pTimestamp("2020-01-01 08:20:00")},
                {KEY_LAST_KEY: pTimestamp("2020-01-01 08:50:00")},
            ],
        ),
    ],
)
def test_by_scale(
    ...
    for i, end_idx in enumerate(end_indices):
        ...
        ) = by_scale(on[start_idx:end_idx], by, buffer=buffer)
        ...
        assert buffer[KEY_LAST_KEY].to_numpy() == buffer_refs[i][KEY_LAST_KEY].to_numpy()
        start_idx = end_idx
